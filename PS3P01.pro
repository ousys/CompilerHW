/*****************************************************************************
  ...
  PS3P01 - Author: Ouyang Siyu (A0077453)
  Modified from TOY COMPILER FOR "WHILE" LANGUAGE
...
:-writeln('Test prgram "test0.s" generated').

Note that if(S), while(S), the S should be boolean expression.If it is
assignment, it will treat is as syntax error and it will not be able to
translate.
...
cs( do { S } while B, Code, Ain, Aout ) :- !,
    cs(S; while B  do {S}, Code, Ain, Aout ).
...
/* runtime-stmt.c -- C file that we need to link in to obtain
        ...
        int main() {
            ...
            // print the result; only significvim ant if eax has been set
