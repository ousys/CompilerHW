...
getLabel(N,List,Elem) :-
	length(List,Len),
	Len > 0, !,             % It is nonsensical to put break or continue outside loop
	(   N =< Len
	->  nth1(N,List,Elem)
	;   getLabel(Len,List,Elem)).

setLabel(N,List,Elem,Out) :-
	length(List,Len),
	Len > 0, !,
	(   N =< Len
	->  nth1(N,List,_,ListAux), nth1(N,Out,Elem,ListAux)
	;   setLabel(Len,List,Elem,Out)).
...
cs(break,Code,Ain,Aout):- !, cs((break 1),Code,Ain,Aout).

cs(break N,Code,Ain,Aout) :- !,
                        % Generate a label name, and generate a jump to that label
	Code = [ '\n\t\t jmp ',Lbl ],
	(   get_assoc(break,Ain,InList), getLabel(N,InList,none)
	->  get_assoc(labelsuffix,Ain,LabelSuffixIn,Aaux,LabelSuffixOut),
	    ...
	    setLabel(N,InList,Lbl,OutList),
	    put_assoc(break,Aaux,OutList,Aout)
	;   get_assoc(break,Ain,InList), getLabel(N,InList,Lbl), Ain = Aout).

cs(continue,Code,Ain,Aout) :- !, cs((continue 1),Code,Ain,Aout).

cs(continue N,Code,Ain,Aout) :- !,
	        % Generate a label name, and generate a jump to that label
	Code = [ '\n\t\t jmp ',Lbl ],
	(   get_assoc(continue,Ain,InList), getLabel(N,InList,none)
	->  get_assoc(labelsuffix,Ain,LabelSuffixIn,Aaux,LabelSuffixOut),
	    generateLabels([Lbl],LabelSuffixIn,LabelSuffixOut),
	        % add the label as an attribute, so that the enclosing 'while'
	        % will know to generate code for that label
	    setLabel(N,InList,Lbl,OutList),
	    put_assoc(continue,Aaux,OutList,Aout)
	;   get_assoc(continue,Ain,InList), getLabel(N,InList,Lbl), Ain = Aout).
...
cs((while B do { S }), Code,Ain,Aout) :- !,
    ...
    get_assoc(break,Ain,OrigBreak,A0,[none|OrigBreak]),
    ...
    get_assoc(continue,A0,OrigContinue,A1,[none|OrigContinue]),
    Cw1 = [ '\n\t\t jmp ',LCond ,
            '\n',LTop,':' ],
    ...
    get_assoc(break,A5,[Break|RestBreak],A6,RestBreak),
    ...
    get_assoc(continue,A6,[Continue|RestContinue],Aout,RestContinue),
...
cs((for (X1=E1;B;X2=E2) do { S }), Code,Ain,Aout) :- !,
    ...
    get_assoc(continue,A5,[Continue|RestContinue],A6,RestContinue),
    ( Continue = none -> Ccontinue = [] ; Ccontinue = ['\n',Continue,':'] ),
    ...
    get_assoc(break,A8,[Break|RestBreak],Aout,RestBreak),
    ( Break = none -> Cbreak = [] ; Cbreak = ['\n',Break,':'] ),
...
compile(P,File) :-
	...
	AbreakIn = Empty,
    put_assoc(break,AbreakIn,[],AbreakOut),
	AcontIn = AbreakOut,                     % initial 'break' label is none
    put_assoc(continue,AcontIn,[],AcontOut),
...
:- compile((
	  ...
	  ,'test0.s'), writeln('Test program "test0.s" generated.').
...
:- compile((
	   ...
	   {
	     x = x + 2;
	     while (x < 5) do {continue 4;};
	     x = x + 4;
	   };
	   x = x
	   )
	  ,'test1.s'), writeln('Test program "test1.s" generated.').
