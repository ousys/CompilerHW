/*****************************************************************************
  CS4212 -- Compiler Design PS4P03
  Student : Ouyang Siyu(A0077453Y)  
  Modified from PS4P02 with compiler_procedures_index.pro
  
  Added Feature from P02:
	  A@(E1:E2)=B@(E:I) (the compiler just check E2 is a valid expression,
	  it does not caculate the value of E2).
	  Notice that it is programmer's responsibility to ensure the logic is correct;
	  the program does not check I-E==E2-E1,
	  it uses the destination size as transfer size. 
	  The result is [A@E1..A@Smaller]=B@[E..I] ,Smaller = min(Size_of_A, I-E+E1)
	  It means copy data from destination to source which starts at E1.  
	  Therefore, if you let E2 >= size of A, or E2-E1\=I-E, the program still runs,
	  but it may cause incorrect display or logical error.
	  However , if you want to copy the range of array B (E..I) to memory pointed by A[E1],
	  you can simply use the syntax : A@(E1:0) = B@(E:I)
	  E1,E,I should be valid expression. It also need to ensure that I-E+E1 < Size_of_A  
  By removing the below code inside cs(A@(E1:E2)=B@(E:I),Code,Ain,Aout) (Line 1622-1623):
		put_assoc(context,Aout,expr,A4),
		comp_expr(E2,_,A4,_),	%just to make sure E2 is a valid expression.
  it will not check E2, which means, E2 can be an invalid expression, i.e. E2=_
  Whether to remove above code or not , it depends on your personal preference. 
...
%
%Implementation simply write AL here, value of E2 is not important , it didn't calculate; 
%Detailed description can be referred to the beginning of this file.
%
cs((A@(E1:E2)=B@(E:I)),Code,Ain,Aout) :-!,atom(A),atom(B),
	...
	append(CE,ECode,RightSideCodeE),
	PushEToStack = ['\n\t\t pushl ', ERegister],
	put_assoc(context,Aout1,expr,A2),
	comp_expr(I,CI,A2,Aout2),
	move_result_to_reg(Aout2,IRegister,ICode),
	append(CI,ICode,RightSideCodeI),
	PushIToStack = ['\n\t\t pushl ', IRegister,
					'\n\t\t popl %ecx',
					'\n\t\t popl %ebx'],
	C2 = ['\n\t\t subl %ebx, %ecx',
    ...
    get_assoc(global_vars,Ain,Globals),
    (	member((B,Ref),Locals)
     ;	member(B,Globals), Ref = B
     ;	atomic_list_concat(['Encountered undeclared variable ',B],Error),writeln(Error), abort),!,
	...
	put_assoc(context,Aout2,expr,A3),
	comp_expr(E1,CE1,A3,Aout3),
	move_result_to_reg(Aout3,CE1Register,CE1Code),
	CpopCode =['\n\t\t pushl ', CE1Register,
				'\n\t\t popl %edx',
				 '\n\t\t jmp ',LCond2 ,
				'\n',LTop2,':' ],
	...
	get_assoc(local_vars,Ain,Locals),
    get_assoc(global_vars,Ain,Globals),
    (	member((A,Ref1),Locals)
     ;	member(A,Globals), Ref1 = A
     ;	atomic_list_concat(['Encountered undeclared variable ',A],Error),writeln(Error), abort),!,
	...
	get_assoc(labelsuffix,Aout3,Kin,Aout,Kout),
    generateLabels([LCond,LTop,LCond2,LTop2],Kin,Kout),
	
	put_assoc(context,Aout,expr,A4),
	comp_expr(E2,_,A4,_),	%just to make sure E2 is a valid expression.
	append([RightSideCodeE,PushEToStack,RightSideCodeI,PushIToStack,C2,CMovePush,Ccondition,CE1,CE1Code,CpopCode,CLeftAss,Ccon2],Code).
...
% Usage example
% d@(k:2*k+2) = b@(1:(k+3)); //transfer size = k+3-1 : (start of index 1 of b) --> (start of index k of d)
% similar effect £º memcpy(address_of_d@k,address_of_b@1,k+3-1) refer to function of memcpy in c

:- Program =
(
	global a@10,i ,b@5,k,d@6;
	...
	d@(k:2*k+2) = b@(1:(k+3));
	b@(2:4) = b@(1:3);
), compile(Program,'test3.s').
:-writeln('The output file is test3.s').